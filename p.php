...
<button onclick="abrirModal('modal1')">Ver publicação</button>

<!-- Modal -->
<div id="modal1" class="modal">
  <div class="modal-conteudo">
    <span class="fechar" onclick="fecharModal('modal1')">&times;</span>
    <h2>Título da Publicação</h2>
    <p>Conteúdo completo da publicação...</p>
...
<?php if (isset($_SESSION['usuario_id'])): ?>
  <div class="edit-photo" onclick="window.location.href='perfil.php?id=<?= $_SESSION['usuario_id'] ?>'">Perfil</div>
<?php endif; ?>
...
$stmtQtdCurtidas = $pdo->prepare("SELECT COUNT(*) FROM curtidas WHERE id_publicacao = ?");
$stmtQtdCurtidas->execute([$pub['id']]);
$totalCurtidas = $stmtQtdCurtidas->fetchColumn();

$stmtQtdComentarios = $pdo->prepare("SELECT COUNT(*) FROM comentarios WHERE id_publicacao = ?");
$stmtQtdComentarios->execute([$pub['id']]);
$totalComentarios = $stmtQtdComentarios->fetchColumn();
...
// Curtir
echo '<form method="POST" action="curtir.php" style="display:inline;">
        <input type="hidden" name="id_publicacao" value="' . $pub['id'] . '">
        <button type="submit"><i class="fas fa-heart"></i> Curtir (' . $totalCurtidas . ')</button>
      </form>';

// Comentar
echo '<button onclick="document.getElementById(\'formComentario' . $pub['id'] . '\').style.display=\'block\'">
        <i class="fas fa-comment"></i> Comentar (' . $totalComentarios . ')
      </button>';

echo '</div>';

// Compartilhar (sem contador)
echo '<form method="POST" action="compartilhar.php" style="display:inline;">
        <input type="hidden" name="id_publicacao" value="' . $pub['id'] . '">
        <button type="submit"><i class="fa-solid fa-share"></i> Compartilhar</button>
      </form>';
...
echo '<form id="formComentario' . $pub['id'] . '" action="comentar.php" method="POST" style="display:none; margin-top: 10px;">
        <input type="hidden" name="id_publicacao" value="' . $pub['id'] . '">
        <textarea name="comentario" placeholder="Digite seu comentário..." style="width:100%; padding:6px;"></textarea>
        <button type="submit">Enviar</button>
      </form>';
$stmtComentarios = $pdo->prepare("
    SELECT c.comentario, u.usuario, u.foto_perfil, c.data_comentario 
    FROM comentarios c 
...
$stmtComentarios = $pdo->prepare("
    ...
    ORDER BY c.data_comentario ASC
");
$stmtComentarios->execute([$pub['id']]);
$comentarios = $stmtComentarios->fetchAll(PDO::FETCH_ASSOC);

foreach ($comentarios as $com) {
    $fotoComent = !empty($com["foto_perfil"]) ? htmlspecialchars($com["foto_perfil"]) : "default.jpg";
    echo '<div class="comment">';
    echo '<img src="' . $fotoComent . '" class="user-avatar" style="width:30px;height:30px;">';
    echo '<div class="comment-text">';
    echo '<strong>' . htmlspecialchars($com["usuario"]) . '</strong><br>';
    echo htmlspecialchars($com["comentario"]);
    echo '<br><small style="font-size:10px;color:#777;">' . date('d/m/Y H:i', strtotime($com["data_comentario"])) . '</small>';
    echo '</div></div>';
}

echo '</div>'; // fecha comments-section
