  ...
  <i class="fas fa-user-friends"></i> Contatos</button>
    ...
    <script>
        ...
        function htmlspecialchars(str) {
            ...
            var map = {
                '&': '&', // Correção para escapar corretamente &
                '<': '<',
                '>': '>',
                '"': '"',
                "'": '\'' // Correção para escapar corretamente '
        ...
        function loadMessages(box = 'entrada') {
            ...
            fetch(`api/obter_mensagens.php?caixa=${box}`)
                ...
                .then(data => {
                    if (data.error) {
                        messageListDiv.innerHTML = `<p style="padding: 20px; text-align: center; color: red;">Erro: ${htmlspecialchars(data.error)}</p>`;
                        return;
                    }
                    ...
                    data.messages.forEach(msg => {
                        ...
                        item.innerHTML = `
                            ...
                            <div class="message-date">${htmlspecialchars(msg.data_envio_formatado)}</div>
                        `;
        ...
        function loadMessageDetail(messageId, box) {
            ...
            fetch(`api/obter_mensagem_detalhes.php?id=${messageId}&caixa=${box}`)
                ...
                .then(data => {
                    if (data.error) {
                        alert('Erro ao carregar detalhes da mensagem: ' + htmlspecialchars(data.error));
        ...
        composeForm.addEventListener('submit', function(event) {
            ...
            .then(response => {
                if (!response.ok) {
                    ...
                    return response.json().then(err => { throw new Error(err.error || 'Erro desconhecido na rede ou servidor.'); });
                }
                return response.json();
            })
            .then(data => {
                if (data.success) {
                    ...
                    composeForm.reset();
                    showPanel('messageList');
                    loadMessages('enviadas'); // Recarrega mensagens enviadas
                    updateUnreadCount(); // Atualiza contagem (caso tenha enviado para si mesmo)
                } else {
                    alert('Erro ao enviar mensagem: ' + htmlspecialchars(data.error || 'Erro desconhecido.'));
                }
            })
            .catch(error => {
        ...
        function deleteMessage(messageId, box) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    loadMessages(box); // Recarrega a caixa atual
                    updateUnreadCount(); // Atualiza a contagem de não lidas
                } else {
                    alert('Erro ao apagar mensagem: ' + htmlspecialchars(data.error || 'Erro desconhecido.'));
                }
            })
            .catch(error => {
        ...
        // Função para atualizar a contagem de mensagens não lidas e notificar
        function updateUnreadCount() {
            console.log('Verificando contagem de mensagens não lidas...');
            fetch('api/obter_contagem_nao_lidas.php')
                .then(response => {
                    if (!response.ok) {
                        console.error('Erro na rede ao buscar contagem de não lidas. Status:', response.status);
                ...
                .then(data => {
                    if (data.error) {
                        console.error('Erro na API de contagem de não lidas:', data.error);
                        return;
                    }

                    const newCount = parseInt(data.count);
                    console.log('Contagem anterior:', previousUnreadCount);
                    console.log('Contagem atual (nova):', newCount);

                    // Atualiza os badges na interface
                    if (newCount > 0) {
                        inboxUnreadCount.textContent = newCount;
                    ...
                    // previousUnreadCount !== null evita som na primeira carga se já houver msg
                    if (newCount > previousUnreadCount && previousUnreadCount !== null) { 
                        console.log('Novas mensagens não lidas detectadas! Tocando som...');
                        playNotificationSound();
                    }
                    previousUnreadCount = newCount; // Atualiza a contagem anterior para o próximo ciclo

                    // Sempre recarrega a lista de mensagens da caixa atual para garantir que o status seja atualizado
                    console.log('Recarregando a lista de mensagens da caixa atual:', currentBox);
                    loadMessages(currentBox);
                })
                .catch(error => {
            ...
            if (notificationSound) {
                notificationSound.play()
                    ...
                    .catch(error => {
                        ...
                        // Isso pode ocorrer se o navegador bloquear autoplay sem interação do usuário
                        alert('Seu navegador pode estar bloqueando o som de notificação. Interaja com a página primeiro.');
                    });
            } else {
                console.log('Elemento de áudio de notificação não encontrado.');
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            // Define o estado inicial da contagem de não lidas para evitar som na primeira carga
            // O ideal seria buscar o valor inicial aqui para 'previousUnreadCount'
            // Por enquanto, ele começa em 0 e só tocará som se aumentar de 0 para >0 ou de N para N+M.
            // Para buscar o valor inicial corretamente:
            fetch('api/obter_contagem_nao_lidas.php')
                .then(response => response.json())
                .then(data => {
                    if (data.count !== undefined) {
                        previousUnreadCount = parseInt(data.count);
                        console.log('Contagem inicial de não lidas carregada:', previousUnreadCount);
                        // Atualiza os badges imediatamente após carregar a página
                        if (previousUnreadCount > 0) {
                            inboxUnreadCount.textContent = previousUnreadCount;
                            inboxUnreadCount.style.display = 'inline-block';
                            sidebarUnreadCount.textContent = previousUnreadCount;
                            sidebarUnreadCount.style.display = 'inline-block';
                        }
                    }
                })
                .catch(error => {
                    console.error('Erro ao buscar contagem inicial de não lidas:', error);
                });

            loadMessages('<?php echo $caixa; ?>'); // Carrega a caixa inicial definida pelo PHP
            messagePollingInterval = setInterval(updateUnreadCount, 15000); // 30000 ms = 30 segundos
        });
        ...
        // Botão "Cancelar" na composição de mensagem
        btnCancelCompose.addEventListener('click', () => {
            loadMessages(currentBox); // Volta para a caixa de mensagens atual
        });

        // Botão "Voltar" do detalhe da mensagem
        btnVoltarListaDetalhes.addEventListener('click', () => {
            loadMessages(currentBox); // Volta para a caixa de mensagens atual
        });
        ...
        if (btnAlterarFoto) { // Verifica se o botão existe antes de adicionar listener
            btnAlterarFoto.addEventListener('click', () => {
                modalAlterarFoto.style.display = 'flex';
            });
        }

        if (closeModal) { // Verifica se o botão existe antes de adicionar listener
        ...
        if (modalAlterarFoto) { // Verifica se o modal existe antes de adicionar listener
