...
try {
    ...
    $stmt = $pdo->prepare("
        UPDATE contatos
        SET status = 'aceito', data_atualizacao = NOW()
    ...
    if ($stmt->execute() && $stmt->rowCount() > 0) {
        ...
        echo json_encode(['error' => 'Convite não encontrado ou você não tem permissão para aceitá-lo, ou já foi aceito/recusado.']);
    }

} catch (PDOException $e) {
    error_log("Erro ao aceitar convite de contato: " . $e->getMessage());
